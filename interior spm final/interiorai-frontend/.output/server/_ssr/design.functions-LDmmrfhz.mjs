...
//#region node_modules/.nitro/vite/services/ssr/assets/design.functions-LDmmrfhz.js
...
var GEMINI_TEXT_MODEL = "gemini-3.8-flash";
var GEMINI_TEXT_FALLBACK_MODEL = "gemini-3.7-flash";
async function fetchGeminiContent({ key, body, model = GEMINI_TEXT_MODEL, fallbackModel }) {
...
var geminiVastuSchema = {
	...
	properties: {
		...
		directionalGuidance: {
			...
			items: {
				...
				properties: {
					...
					traditionalAssociation: { type: "STRING" },
					suggestedUse: { type: "STRING" },
					placementGuidance: { type: "STRING" },
		...
		priorityActions: {
			...
			required: ["highPriority", "optional"]
		},
	...
	required: [
		...
		"considerations",
		"priorityActions",
		"practicalInteriorNote"
	]
};
function parseVastuGuidance(content) {
	const parsed = JSON.parse(content);
	if (!parsed.summary || !parsed.priorityActions || !parsed.practicalInteriorNote || !Array.isArray(parsed.directionalGuidance) || !Array.isArray(parsed.furniturePlacement)) throw new Error("Gemini returned an invalid Vastu guidance plan. Please try again.");
	return parsed;
}
function parseBudgetPlan(content, requestedBudget) {
	...
	if (!parsed.summary || !overview || categories.some((value) => !Number.isFinite(value) || value < 0)) throw new Error("Gemini returned an invalid budget plan. Please try again.");
...
var generateVastuGuidance = createServerFn({ method: "POST" }).inputValidator((input) => VastuInput.parse(input)).handler(generateVastuGuidance_createServerFn_handler, async ({ data }) => {
	...
	const response = await fetchGeminiContent({
		...
		body: {
			...
			generationConfig: {
				responseMimeType: "application/json",
				responseSchema: geminiVastuSchema
			}
		}
	});
	if (!response.ok) {
