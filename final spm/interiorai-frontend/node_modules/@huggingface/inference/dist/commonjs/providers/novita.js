...
exports.NovitaTextToVideoTask = exports.NovitaImageTextToVideoTask = exports.NovitaConversationalTask = exports.NovitaTextGenerationTask = void 0;
/**
...
const NOVITA_MINIMAX_H3_RATIOS = ["21:9", "16:9", "4:3", "1:1", "3:4", "9:16"];
...
class NovitaImageTextToVideoTask extends providerHelper_js_1.TaskProviderHelper {
    constructor() {
        super("novita", NOVITA_API_BASE_URL);
    }
    makeRoute(params) {
        if (params.model !== "MiniMax-H3") {
            throw new errors_js_1.InferenceClientInputError(`Unsupported Novita image-text-to-video model: ${params.model}`);
        }
    ...
    preparePayload(params) {
        ...
        if (!prompt?.trim()) {
            throw new errors_js_1.InferenceClientInputError("Novita MiniMax H3 requires a non-empty parameters.prompt.");
        }
        return {
            ...(0, omit_js_1.omit)(restParameters, ["guidance_scale", "negative_prompt", "num_inference_steps", "seed"]),
            model: params.model,
            content: [
    ...
    async preparePayloadAsync(args) {
        ...
        return {
            ...args,
            inputs: await (0, dataUrlFromBlob_js_1.dataUrlFromBlob)(args.inputs, args.inputs.type || "image/png"),
        };
    }
    async getResponse(response, url, headers, _outputType, signal) {
        if (!url || !headers) {
            throw new errors_js_1.InferenceClientInputError("URL and headers are required for Novita API calls");
        }
        ...
        while (true) {
            ...
            if (!resultResponse.ok) {
                throw new errors_js_1.InferenceClientProviderApiError("Failed to fetch response status from Novita API", { url: resultUrl, method: "GET", headers }, {
            ...
            switch (task.status) {
                case "succeeded": {
                    if (!task.content?.url) {
                        throw new errors_js_1.InferenceClientProviderOutputError("No output URL returned by Novita API");
                    }
                    const videoResponse = await fetch(task.content.url, { signal });
                    if (!videoResponse.ok) {
                        throw new errors_js_1.InferenceClientProviderApiError("Failed to fetch generation output from Novita API", { url: task.content.url, method: "GET" }, {
                ...
                case "failed":
                case "cancelled":
                    throw new errors_js_1.InferenceClientProviderOutputError(task.error?.message || `Novita task ${task.status}`);
                default:
                    await (0, delay_js_1.delay)(5000, signal);
            }
        }
    }
}
exports.NovitaImageTextToVideoTask = NovitaImageTextToVideoTask;
