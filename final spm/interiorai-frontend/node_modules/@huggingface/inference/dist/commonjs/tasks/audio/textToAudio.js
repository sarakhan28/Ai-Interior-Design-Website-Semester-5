"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.textToAudio = textToAudio;
const getInferenceProviderMapping_js_1 = require("../../lib/getInferenceProviderMapping.js");
const getProviderHelper_js_1 = require("../../lib/getProviderHelper.js");
const makeRequestOptions_js_1 = require("../../lib/makeRequestOptions.js");
const request_js_1 = require("../../utils/request.js");
/**
 * This task generates audio (e.g. music or sound effects) from an input text prompt.
 * Example model: stabilityai/stable-audio-open-1.0
 */
async function textToAudio(args, options) {
    const provider = await (0, getInferenceProviderMapping_js_1.resolveProvider)(args.provider, args.model, args.endpointUrl, options);
    const providerHelper = (0, getProviderHelper_js_1.getProviderHelper)(provider, "text-to-audio");
    const { data: res } = await (0, request_js_1.innerRequest)(args, providerHelper, {
        ...options,
        task: "text-to-audio",
    });
    const { url, info } = await (0, makeRequestOptions_js_1.makeRequestOptions)(args, providerHelper, { ...options, task: "text-to-audio" });
    return providerHelper.getResponse(res, url, info.headers, undefined, options?.signal);
}
