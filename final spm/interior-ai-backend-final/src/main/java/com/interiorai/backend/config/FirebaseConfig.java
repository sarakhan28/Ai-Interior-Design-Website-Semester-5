...
public class FirebaseConfig {
    ...
    private synchronized FirebaseApp getOrInitFirebaseApp() {
        ...
        try {
            ...
            // No explicit credentials supplied. Only attempt to reach out for Google
            // Application Default Credentials (which performs a network/metadata-server
            // lookup and can be slow or hang in offline/CI/test sandboxes) when we are
            // NOT running in mock mode. In mock mode we skip this network attempt
            // entirely so the Spring context starts fast and deterministically -
            // this was the root cause of slow/flaky test bootstraps.
            if (!mockEnabled) {
                FirebaseOptions options = FirebaseOptions.builder()
